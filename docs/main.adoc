...
Build all services in order based on sequence in services/build.edn, then deploy 3 tiers of service to live servers.
    ...
    concierto deploy \
        ...
        --target live \
        --select \ 
            cluster live role db also \
            cluster live role api also \
            cluster live role balancer 
