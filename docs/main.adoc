...
The typical usage as of now, is to run a concierto script in the application directory ...
...
=== Configuration Settings
    ...
    ./concierto set scenario cluster
...
Build and deploy all services for role api to cluster stage (without output)

You build for a target as the target contains the service versions from services.edn, and the registry url ...

    ./concierto build --role api --push
    ./concierto deploy \
         --scenario single \
         --select role local

Build all services in order based on sequence in services/build.edn, then deploy 3 tiers of service to staging servers.
...
Run it remotely, but with pre upload templating ...

[source, bash]
----
./concierto remote --target live --select cluster west -- scripts/docker-login
----
...
Roles are aggregated as *scenarios*. For example, a test scenario could apply machine limits; memory, cpu etc to running services, whereas a production scenario may be unlimited. Or maybe different scenarios per customer using your app. Scenarios allow flexibily switching sets of  docker-compose files, favouring declarative verbosity over embedding template logic for multiple variations (although that's still possible).
...
These files will be described in <<_details>> in detail, but the basic functioning of Concierto is to aggregate all data generated from these files into a map (keyed by filename) for a given target and then use the data for templating during any subsequent process.
...
One potential negative is the fact your Dockerfiles have non-standard templating and you can't run them independently. This can be solved by templating your Dockerfiles on the fly. e.g. ...
...
Concierto comes as a jar file and a script, and I typically execute it in the directory of the app itself, so that it is versioned along with all other files.

Until a better idea appears, get the concierto bootstrap script, preferably into a new empty directory ...

    wget https://www.conciert.io/release/concierto
    chmod +x concierto

and then make it executable, thereafter you can get the latest jar file with ...
...
In the example below, given a cluster name, the attr-cluster function find the db machine of the cluster and the api machines of the cluster, and creates keys which will be merged into the final attribute set under the :on-cluster key.
